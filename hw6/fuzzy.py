...
MAX_OFFSET   = 320         # 對應畫面一半寬度，用於正規化 e (-1~1)
...
def detect_lane(frame: np.ndarray) -> tuple[float | None, np.ndarray]:
...
try:
    ...
    for i, en in enumerate(names):
        for j, den in enumerate(names):
            out_idx = 6 - max(min(i + j - 6, 6), 0)  # 手感 mapping
    ...
    def fuzzy_controller(e: float, de: float, kp: float = 1.5, kd: float = 0.5):
        return np.tanh(kp * e + kd * de)  # 限幅 -1~1
...
async def camera_loop(cap: cv2.VideoCapture, q: asyncio.Queue, cfg: dict):
    """Compute error, put into queue as (e, timestamp)"""
    w = cfg["width"]
    max_offset = w / 2
    e_prev = 0
    while True:
        ...
        if lane_x is None:
            await asyncio.sleep(0.01)
            continue
        offset = (w / 2 - lane_x) / max_offset  # -1~1
        de = offset - e_prev
        e_prev = offset
...
def main():
    ...
    parser.add_argument("--camera", default=DEFAULTS["camera"],
                        help="相機編號或影片路徑")
